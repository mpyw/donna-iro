    ...
    pub fn find(&self, raw: &str) -> Option<Answer> {
        ...
        (0..parts.len()).find_map(|i| {
            ...
            match self.nearest(single) {
                ...
                //
                // ただし**読みで終わるときだけ**。途中でまたぐものまで
                // 拾うと、フィラーの尻と答えの頭が繋がって別の色になる。
                //
                // ```text
                // じゃあ おれん => じゃ|あお|れん → あお
                // ```
                //
                // 「おれん」は言い切りとして i+1 で拾えるのに、ここが
                // 先に潰していた。フィラーが「あ」で終わり答えが母音で
                // 始まる組は、全部この形になる。
                None => joined
...
fn assemble(raw: &str, head: usize) -> Vec<String> {
    ...
    for seg in segments(&normalize(raw)) {
        ...
        if QUESTION.contains(seg.as_str()) {
            ...
            if joinable {
                // **出来上がっている答えの上には貼らない。** 幻覚の継ぎ足しが
                // 「き いろ」と割れて届くと、「あか」に貼り付いて「あかきいろ」に
                // なり、長い読みを優先する部分一致で きいろ が勝つ。
                //
                // ```text
                // あか き いろ => きいろ
                // ```
                //
                // 先頭の完全一致が**組み立ての段階で**消えるので、位置を
                // 最優先する仕掛けでは救えない。貼る先が読みそのものなら、
                // それはもう答えなので触らない。
                // 完全一致だけを見ていた頃は、語尾が付くと素通りしていた。
                //
                // ```text
                // あかだよ き いろ => きいろ
                // ```
