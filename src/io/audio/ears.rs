...
impl Ears {
    ...
    pub fn listen(&mut self, max: Duration) -> Result<Option<Vec<f32>>> {
        ...
        // ストリームは鳴っている間も回り続けているので、直前に流した歌が
        // 溜まっている。**末尾だけ残して捨てる。**
        //
        // 全部捨てていた頃は、窓が開くのは歌が鳴り止んでから。子どもは
        // 「どんないろがすき」の「すき」あたりから被せて答えるので、
        // **答えの頭が録れていなかった。** 実機のログに出た形はどれも頭が
        // 欠けている（「んぶ」←ぜんぶ、「より」←みどり、「たん」←あか）。
        // 判定の側では、欠けたものを足すことはできない。
        //
        // 残すぶんには質問の尻（「…がすき」）も入るが、回り込みは判定の側で
        // 落とす（`matcher.rs` の QUESTION）。もともと whisper が
        // `initial_prompt` の質問文を吐き返すので、その道は前からある。
        //
        // **判定の窓は動かさない。** 開ける時刻も `guard` もそのままなので、
        // 残響で誤発火する側は変わらない。増えるのは録れている音だけ。
        //
        // 裏を返すと、**歌の裏で言い切ってしまった声は拾えない。** 声が
        // 続いていること自体は `guard` のあとで見るので、そこまでに黙って
        // いれば「無言」になる。歌に被った音は歌と区別が付かないため、
        // ここでは見分けようがない。詰めるなら `guard_ms` の側。
        {
            let mut b = lock(&self.buf);
            let keep = (self.rate as f32 * self.cfg.preroll().as_secs_f32()) as usize;
            let drop = b.len().saturating_sub(keep);
            b.drain(..drop);
        }
